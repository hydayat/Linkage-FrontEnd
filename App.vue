		...
		<Layout>
			...
			<Layout>
				<Sider hide-trigger :style="{background: '#fff'}">
					<Menu ref="menu" :active-name="activeName" theme="light" width="auto" :open-names="['1','2','3','4']" v-if="islog"
					 @on-select="select">
						<Submenu name="1">
							<template slot="title">
						...
						<Submenu name="2">
							...
							Invitation
							<Badge text="new" type="warning" style="margin-left: 35px;margin-bottom: 5px;" v-if='invitationBadge == 1? true : false' />
							</MenuItem>
							<MenuItem name="2-2" @click.native="chat()">
							Chat
							<Badge text="new" style="margin-left: 61px; margin-bottom: 5px;" v-if='messageBadge == 1?true:false' />
							</MenuItem>
						</Submenu>
						<Submenu name="3">
							<template slot="title">
				...
				<Layout :style="{padding: '0 24px 24px'}">
					...
					<router-view @child-event="parentEvent" @changeActiveName="changeActiveName" @submitMessage="onMessageSubmit"
					 @unreadMessage="readUnreadMessage" @unreadAddFriendRequest="readUnreadAddFriendRequest" @unreadAddFriendReply="readUnreadAddFriendReply"
					 @submitInvitation="onSubmitInvitation" @submitReply="onSubmitReply" @connectWebSockect="connectWebSockect"
					 @clearMessageBadge="clearMessageBadge" @clearInvitationBadge="clearInvitationBadge"></router-view>
				</Layout>
			</Layout>
		</Layout>
	</div>
			...
			showGreeting(message) {
				...
				for (var i = 0, len = chatData.chatList.length; i < len; i++) {
					if (chatData.chatList[i].name == message.name) {
						...
						if (chatData.current[i].name != message.name) {
